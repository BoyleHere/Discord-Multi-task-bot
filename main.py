...
def mailgun_send(email_address, verification_code):
  return requests.post(
	 ...
	 data={"from": "Prashast but Bot <mailgun@{}>".format(os.environ['MAILGUN_DOMAIN']),
		  "to": email_address,
	  	"subject": "Verify your E-Mail for AIS Official Server",
...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game(name='.vstatus | Bot By Boyle'))
...
async def on_message(message):
        ...
        if len(users_guilds) >= 1:
            ...
            if len(verif_list) >= 1:
                ...
                emailmessage = Mail(
                    ...
                    subject='AIS Official server verification',
                    html_content= '<strong>Verification code: </strong>'+str(random_code))
                try:
    ...
    elif 'F' is message.content:
      await message.channel.send('F')
      
    elif 'bruh' in message.content:
      await message.channel.send('bruh -_-')

    elif '<@!334009996323061760>' in message.content or '<@!332856405939978251>' in message.content:
      if 'Mods' in [role.name for role in message.author.roles]:
        return
      elif 'FOUNDERS' in [role.name for role in message.author.roles]:
        return
      elif 'Jr. Mods' in [role.name for role in message.author.roles]:
        return
      elif 'Sr. Mods' in [role.name for role in message.author.roles]:
        return
      else:
        await message.channel.send('You cannot ping the Owner!! <@{0.author.id}>'.format(message))
        await message.delete()
        fmt = '@{0.author}\'s message has been deleted for pinging the Gods: `{0.content}`'
        channel = client.get_channel(886113417981734913)
